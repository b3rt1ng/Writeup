...
### Page principale : mcp.kobold.htb

La navigation sur `https://kobold.htb` révèle un lien direct vers `mcp.kobold.htb` (MCPJam Inspector).
...
- `bin.kobold.htb` → instance PrivateBin
...
Interface : **Arcane v1.13.0**, un gestionnaire Docker avec accès au socket `unix:///var/run/docker.sock`.
